...
void Config::cli_parse(int argc, char **argv) {
  ...
  app.add_option("--mtrace-range", mtrace_ranges, "Specify memory tracing range (default: 0x80000000-0x8fffffff)")
      ->delimiter(',');
  ...
  for(auto const& range: mtrace_ranges) {
    std::cout << std::hex << range.first << ',' << range.second << std::endl;
  }
}
